...
**Why SQLite for the deltas:** the dataverse is a filesystem of small documents — thousands of little rows, read far more often than written, where you must always see your own write the instant you make it. SQLite is exactly that shape: each file is one row (`namespace`, `key`, `value`), reads are consistent point lookups rather than eventually-consistent snapshots, and the schema is one `CREATE TABLE` you never have to think about again. It also requires no credit card, scales to 5 GB and millions of daily reads on the free tier, and hard-caps at the limit instead of billing. The baseline stays in assets (free, unlimited, served from the edge) and the browser cache stays in IndexedDB — the database is the thin living layer in between.
